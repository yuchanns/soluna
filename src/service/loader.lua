...
function S.load(filename, offx, offy, x, y, w, h)
	...
	if cx then
		subimg = image.canvas(c.data, c.w, c.h, ( x or 0 )+ cx, ( y or 0 ) + cy, cw, ch)
	end
	local id = #sprite+1
	sprite[id] = { filename = filename, index = index, id = id, data = subimg, x = offx, y == offy }
	return id
end
...
function S.write(id, filename)
	local obj = sprite[id]
	assert(obj.data)
	local img = image.new(image.canvas_size(obj.data)) 
	image.blit(img:canvas(), obj.data)
	img:write(filename)
end
