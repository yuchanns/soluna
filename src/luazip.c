...
static int
skip_bytes(zipreader_file zf, ptrdiff_t offset) {
...
int
zipreader_seek(zipreader_file zf, ptrdiff_t offset, int origin) {
	...
	case SEEK_SET:
		...
		break;
	case SEEK_CUR:
		new_pos = (ptrdiff_t)cur_pos + offset;
		break;
	case SEEK_END:
		new_pos = (ptrdiff_t)size + offset;
		break;
	default :
		return -1;
	}
